package io.bitbucket.plt.sdp.bohnanza.State;

import io.bitbucket.plt.sdp.bohnanza.Cards.BeanCard;
import io.bitbucket.plt.sdp.bohnanza.Deck.DeckModel;
import io.bitbucket.plt.sdp.bohnanza.Game.GameController;
import io.bitbucket.plt.sdp.bohnanza.Player.PlayerController;
...
public class DrawBeanCardsState implements State{
    ...
    private void addCardsToHandFromDeck(DeckModel deckModel, Optional<PlayerModel> playerModel) {
        ...
        for (int i = cardsToAdd.size() - 1; i >= 0; i--) {
            playerHand.addFirst(cardsToAdd.get(i));
        }
    }
